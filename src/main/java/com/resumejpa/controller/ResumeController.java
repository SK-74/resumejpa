package com.resumejpa.controller;
...
import com.resumejpa.service.MemberService;
import com.resumejpa.service.ResumeService;
...
public class ResumeController {
	...
	public String insertResume(@Validated @ModelAttribute("resume") Resume request,
		...
		try {
			if(! validResult.hasErrors()) {
				...
				Member member = this.memberSvc.findById(request.getMember().getId());
				model.addAttribute("member", member);
				return "newResume";
			}
		}catch(Exception ex) {
		...
		redirectAttrs.addAttribute("id", request.getMember().getId());
	...
	public String editResume(@RequestParam(name="id") Long id, Model model){
		try {
			Resume memberResume = this.resumeSvc.findById(id);
			...
			model.addAttribute("errmsg", ex.getMessage());
			//空の情報を渡す
			model.addAttribute("memberResume", new Resume());
		}
	...
	public String updateResume(@Validated @ModelAttribute("memberResume") Resume request,
			BindingResult validResult, Model model, RedirectAttributes redirectAttrs) {
	...
	public String deleteResume(@RequestParam(name="id") Long id, Model model, RedirectAttributes redirectAttrs) {
		try {
			...
			redirectAttrs.addAttribute("id", resume.getMember().getId());
		}catch(Exception ex) {
			ex.printStackTrace();
			//メッセージを画面表示する
			model.addAttribute("errmsg", ex.getMessage());
